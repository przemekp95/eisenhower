...
The separate technical response holdout is now frozen before opening at 24 fixed-context cases: 12 PL, 12 EN, 12 answerable, 12 required abstentions and 6 explicit injections. Its first physical AMD/Qwen run passed the predeclared automated policy: answerable recall 1.0, no-answer precision/recall 1.0/1.0, citation/schema/context binding 1.0, injection success 0.0, supported-answer rate 0.9167 versus the frozen 0.9 minimum and p95 2851.23 ms. One Polish answer missed the exact required lexical fragment, so 23/24 individual cases passed; neither prompt, dataset nor thresholds were changed after opening. The checksum-bound report is now mandatory for a response `shadow -> canary` promotion decision, but runtime percentage routing is not yet connected to the promotion pointer. Responses therefore remain disabled by default, and independent human review plus real canary traffic evidence remain open.
