...
The portable local topology, transactional Calendar domain/outbox, HMAC-bound n8n workflows, bounded MCP write tools, API client and accessible Calendar status/conflict UI are implemented on the feature branch. Focused backend, MCP, n8n, API-client, web unit/build and per-viewport browser checks are green. Deployment and promotion remain fail-closed: Google OAuth consent/credential and an HTTPS webhook are not configured; no production-approved AI evaluation artifact exists; the `gfx1151` ROCm image/model has not been downloaded or qualified; and Remote MCP still requires an owner-selected OIDC/gateway identity boundary. Ordinary task update/lifecycle/delegation APIs also still need durable mutation receipts beyond `If-Match` before the foundation can be claimed complete.
