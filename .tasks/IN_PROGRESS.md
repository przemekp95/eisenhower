...
## TASK-062: Build the governed AWS production core
**Priority:** P0 | **Tags:** aws, cdk, postgresql, redis, s3, ci-cd, operations

Implement a real, evidence-bounded AWS platform path for the Node API and web application. Use CDK v2 in TypeScript, migrate persistence incrementally behind existing ports, give Redis and S3 real multi-instance/product responsibilities, and replace static AWS keys with environment-scoped GitHub OIDC. Keep paid provisioning and production promotion behind explicit cost and authorization gates.

### Plan

- Audit the current architecture and job requirements; record the ADR, gap map, cost model, rollback boundaries, and evidence vocabulary.
- Establish the shared repository verification gate and RED-GREEN infrastructure assertions.
- Implement and test the CDK staging/production stacks plus PostgreSQL/Prisma, Redis, and S3 application adapters incrementally.
- Implement and test GitHub OIDC validation, staging auto-deploy, exact-green-SHA manual production release, smoke, rollback, and evidence capture.
- Stop at the cost gate before provisioning; after explicit approval and credentials, deploy staging and execute the operational drills.
- Only after runtime evidence exists, prepare strictly qualified CV bullets distinguishing implementation from production ownership.

### Progress

The local production-core path is implemented: environment-scoped CDK identity/registry/platform stacks, private PostgreSQL/Valkey/S3 boundaries, exact-SHA stopped-first migrations, restricted runtime and operator roles, Prisma and incremental MongoDB migration adapters, shared Redis coordination, immutable S3 artifacts, alarms, cost gate, runbook, and staging/production workflows. Fresh local evidence includes 36/36 CDK assertions plus offline synthesis for both environments, 288/288 Node tests against real PostgreSQL and Redis, clean Node and AWS typechecks/builds, zero high-severity Node or AWS-ops dependency findings, S3 adapter tests, workflow actionlint/contracts, and a production image contents check. The owner approved one non-renewing staging exercise capped at USD 38 before VAT through 2026-09-15T10:32:08+02:00, with an explicit stop before selecting or confirming any payment card, and bound it to their single shared AWS account `802954286692` in `eu-central-1`. The identity design now synthesizes one shared GitHub OIDC provider with separate staging/production subjects and roles; identity, staging and production use the isolated bootstrap qualifiers `eisnidt`, `eisnstg` and `eisnprd`. Existing non-Eisenhower resources remain out of scope, and same-account qualifiers are documented as weaker than separate account boundaries. The owner-authorized tax registration correction was submitted using the current MF/VIES taxpayer identity, active EU VAT TRN and legal address. AWS reports verification in progress and still renders the prior table values, so tax display acceptance remains open until the verified read-back changes; no payment method was viewed or edited. The owner approved hardened same-account bootstrapping: every synthesized role now carries its identity/staging/production boundary, while a fail-closed plan generator requires exact customer-managed CloudFormation execution-policy ARNs and refuses the default `AdministratorAccess` path. The locally configured `pp-solutions-render-storage` credentials were verified as the correct account but denied CloudFormation/IAM inspection and remain forbidden for deployment. Account bootstrap, policy read-back and the shared OIDC identity stack are complete; application deployment, ARM64 execution, alert delivery, restore/rollback drills, and runtime acceptance remain explicitly open. `pypdf` is updated to patched 6.16.1. The owner-approved NLTK exception accepts only transitive `nltk==3.10.3` with sole advisory `PYSEC-2026-3740` while no fixed release exists, forbids project-owned NLTK imports, and expires fail-closed on any fix, advisory or dependency drift. When the final promotion audit newly reported `CVE-2026-69112`, the owner approved the same fail-closed treatment solely for transitive `accelerate==1.14.0` through `unstructured-inference==1.6.13`; project-owned Accelerate imports and both affected checkpoint-loading APIs are forbidden, and any fixed release, second advisory or graph drift expires the exception. The live production audit checks 202 dependencies successfully under these bounded policies. The owner also approved replacing the artificial global 100% Node coverage threshold with a contract-enforced 95% statements/functions/lines and 90% branches while excluding only generated Prisma code and the subprocess-tested migration CLI; the measured TASK-062 suite now reaches 96.93%/95.01%/97.75%/94.21% respectively with all 288 tests green. Patched `xmldom` 0.8.15/0.9.12 overrides remove the moderate advisory from web/mobile audits, and the new AWS infrastructure CI context is synchronized across impact planning, required-status bridging, master-to-dev synchronization, release preflight and acceptance documentation.

Latest external evidence supersedes the earlier open policy/bootstrap note: the existing MFA-protected root console session created six Access Analyzer-validated customer-managed policies/boundaries and three version-32 CDK bootstrap stacks in `eu-central-1`. A live first-run check proved CDK's boundary flag covers only the CloudFormation execution role, so the path now generates a pinned custom template; AWS final read-back confirms the identity/staging/production stacks are complete, 15/15 bootstrap roles carry their correct boundary, and all asset buckets are EU-region, KMS-encrypted and public-blocked. The shared `Eisenhower-GitHubOidc` stack is now `CREATE_COMPLETE` with one native provider and two environment-scoped, boundary-constrained roles; the custom-resource Lambda/helper-role variant was rejected under RED-GREEN rather than widening IAM. GitHub environment `staging` has the exact account, role, certificate, origin, OIDC and AI variables with its one-off deployment gate enabled. Cloudflare authoritatively serves `eisenhower-auth.pietrzakprzemyslaw.pl` and `eisenhower-ai.pietrzakprzemyslaw.pl` through the existing Warsaw-connected tunnel to the owner-operated runtime in Poland; direct edge checks returned correct issuer/JWKS metadata plus healthy AI and Node readiness. Keycloak preserves the local callback and admits the future `eisenhower-staging.pietrzakprzemyslaw.pl` callback. ACM issued the correct first-level staging certificate, and the unused nested-name certificate and validation record were removed. The web ECS task now receives its required API, AI and OIDC runtime contract under a RED-GREEN assertion. Master and dev reached the same exact-green `1ea62f1c009e11f669ae98d4ef08c286444af9d3`; its first manual dispatch was safely skipped before a runner or AWS call because GitHub environment variables are unavailable in a job-level condition. A RED-GREEN workflow contract now requires a least-privilege staging-environment gate job to resolve that variable and permits the OIDC deployment job only through its explicit `enabled=true` output. Application deployment remains pending promotion of this correction and a repeated exact-green dispatch. Production, card access and any spend beyond USD 38/seven days remain closed, and `pp-solutions-render-storage` remains forbidden for deployment.
...
- Keep grounded answers single-turn with sources, preview, and apply-to-description; route capabilities from the actual knowledge runtime, apply the 2026-08-19 checksum-bound owner decision only after the new manifest/projection/generator/holdout/rollback gates pass, and keep every MAG and memory capability disabled.
...
### 2026-08-19 private RAG and OIDC evidence

- Exact green master `e4fb48e5` was exercised in an isolated blue-green runtime without final traffic activation: the approved 19 canonical documents and 150-point BGE-M3 projection were restored from checksum-bound evidence, the Qdrant alias cutover was durably audited, pinned Qwen/BGE providers were healthy, and signed response promotion revision 5 enabled only the explicit owner cohort through 2026-09-18 while MAG and all memory phases remained disabled. Authenticated private HTTPS returned a grounded FastAPI answer with its repository citation and a separate correct no-answer with no citations; routing rollback to the prior `9090` runtime passed. Runtime failures exposed and TDD fixed numeric AMD group IDs, missing `INFERENCE_MODEL` propagation and promotion-pointer read permissions. Full `make verify` is green, but this hotfix still requires protected exact-SHA promotion, final image scan/SBOM, readiness-bound n8n activation and final routing before deployment completion.
- OIDC entry is test-bound to one automatic PKCE authorization attempt; callback handling remains intact, OIDC rejection exposes translated retry plus language selection, and the manual token gate remains limited to non-OIDC mode. Web unit, integration and production build checks passed; live browser acceptance remains pending the final runtime.
- The approved 19-file corpus manifest now records every source SHA/size and snapshot `35fc904bf027f4074821bbc1d4ef1e16a7b3806c1953085b3ac35db11071f609`. The 42-case v4 packet is checksum-bound and explicitly pending independent label review rather than reusing stale v3 approval.
- Isolated BGE-M3 plus authenticated BGE reranker RAGOps on clean SHA `0cb162257b05415469069e1c1d9cfd7c1d6f384b` accepted 19 documents, produced 150 points, reconciled with zero missing/stale/orphan records, and repeated ingestion as 19 duplicates with unchanged 19/150 counts. Snapshot restore matched, cleanup passed, global recall/MRR were 0.9107/0.8286, PL 0.8571/0.7976, EN 0.9643/0.8595, no-answer accuracy 1.0, and stale/forbidden/isolation rates 0.
- A prefinal physical AMD Qwen diagnostic on clean SHA `7487a7c8d775b55d25670e15689dbedfa3f18b5a` was green across the 24-case knowledge-answer holdout: answerable recall 1.0, no-answer precision/recall 1.0, citation correctness and coverage 1.0, schema validity 1.0, injection success 0, and local p95 2.72 seconds. It is not final-image evidence and preserves `human_review.satisfied=false` and `production_quality_proven=false`.
- A new non-root Chainguard/Wolfi response runtime replaces the vulnerable inherited vLLM/Ubuntu image while preserving the private OpenAI chat/score contracts. The physical AMD probe runs pinned Qwen3 and BGE revisions concurrently with read-only caches, all Linux capabilities dropped, no-new-privileges and no host IPC; Trivy reports zero findings across UNKNOWN/LOW/MEDIUM/HIGH/CRITICAL. Its bounded schema enforcement and at most two corrective retries produced a green 24/24 PL/EN answer/no-answer/injection holdout with 100% supported answers, citations, schema validity and context binding, 0% injection success and 5.93 s p95. Fresh full verification passed Node 276, web 254 at 100%, BDD 21/149, API 34, MCP 50, n8n 20, AI 874 passed/13 skipped at 87.47%, mobile 202, dependency audits, builds, typecheck and Pylint 10.00/10. This remains prefinal probe/source evidence only; exact final-SHA rebuild, scan/SBOM, activation receipt, deployment smoke and rollback are required.
- The private response image contract, read-only model caches, live-readiness-bound n8n reconciliation, credential importer, activation receipt, and private deploy preflight are implemented. Generation/response, both RAG n8n workflows and all deployment routing remain inactive pending final-SHA verification, image scan/SBOM, repeated holdouts, authenticated runtime smoke and rollback rehearsal. MAG and all memory flags remain disabled.
- The complete feature-candidate verification is green after closing OIDC recovery and exact response-image contract coverage: Node 276, web 254 at 100% coverage, BDD 21 scenarios/149 steps, API client 34, MCP 50, n8n 20, AI 867 passed/13 skipped at 88.05% coverage, mobile 202, production dependency audits, builds, typecheck and Pylint 10.00/10. This is source verification only; exact promoted-SHA artifact, deployment and traffic evidence remain separate gates.
...
Knowledge capabilities are now derived from the authenticated RAG runtime, generator, response flag, allowlists and canary decision rather than classifier readiness. Classifier, OCR and bulk labels state MiniLM embedding + PyTorch/MLP and Tesseract truthfully. The existing local private runtime remains OIDC/allowlisted with retrieval true only in the knowledge role, generation/response and all memory flags false. Its approved canonical collection is empty; the current manifest SHA no longer matches the SHA frozen in the owner packet, so corpus ingest and any generation/MAG activation remain fail-closed pending a fresh checksum-bound decision. The stale browser journeys that blocked the first PR run now follow Integrations and saved-task assistance, persisted test data is isolated across viewport projects, and focus returns to the task card after an in-dialog update. Exact-head CI measured valid narrow-runner mobile flows at 64–76 seconds; the affected scenario budgets are therefore bounded at 60/90 seconds without changing the 20-minute job budget, retries, Axe checks or assertions. All 24 Playwright/Axe checks and fresh full verification are green: Node 276 and web 244 tests at 100% coverage, BDD 21 scenarios/149 steps, API client 34, MCP 50, n8n 18, web integration 2, AI 833 passed/13 skipped at 87.93% coverage, mobile 202, production dependency gates, Node typecheck and pylint 10/10. The first exact-SHA promotion reached synchronized green master/dev at `e2eff0631213d3b2fd89ea748893a699b3fac996`; its isolated blue-green start then exposed deploy-only failures that source CI had not exercised. Focused red-green runtime contracts now require all non-root first-party writers to wait for initialized writable volumes, initialize every runtime volume as uid/gid 65532, preserve the approved artifact group for the ingest worker, and expose the MCP console script from `/opt/python/bin`. Re-promotion, rebuilt-image security evidence and final local activation remain pending.

On 2026-08-19 the owner separately approved private activation of the full single-turn grounded response for the existing explicit user allowlist, subject to a freshly regenerated checksum-bound corpus manifest, final-SHA activation receipt, isolated canonical/projection checks, pinned private generator/reranker, physical answer/no-answer evaluation, live n8n readiness, stop thresholds and rollback rehearsal. The same decision requires automatic Keycloak redirect in OIDC mode and an error-only localized retry surface; non-OIDC manual tokens remain supported. Plans are recorded in `docs/superpowers/plans/2026-08-19-task-065-oidc-entry.md` and `docs/superpowers/plans/2026-08-19-task-065-private-rag-delivery.md`. No runtime flag, corpus, workflow or route has been changed by planning; MAG and memory remain out of scope and disabled.

OIDC entry now starts the existing S256 PKCE authorization flow automatically and exactly once per attempt, including React Strict Mode. A synchronous session marker plus one shared in-flight Promise prevents redirect races; stale attempts and callback/start failures lead to a localized retry surface with `LanguageSwitcher`. OIDC recovery contains no manual-token, access-code or card-memory copy, while incomplete/non-OIDC configuration retains the in-memory manual bearer-token gate. Recorded RED runs failed on the missing guard/reset and the old click-to-login/rejected-code UI; fresh GREEN evidence is 9/9 `oidcSession`, 30/30 focused App/session, 250/250 full web unit tests, 2/2 web integration tests, production build and Prettier check. This is source/test evidence only; no Keycloak browser journey or runtime was changed by this slice.
