...
| --- | --- | --- |
| LOCAL | green for the current candidate worktree, excluding external release credentials | `make verify` passed: Node 65 tests at 100%, web 125 unit plus 2 integration tests at 100%, AI 189 passed/2 explicitly skipped at 89% coverage, and mobile 92 tests above its coverage gates. Playwright passed 2/2 against an isolated real Node/Mongo stack. Root and Mikrus Compose configurations validated with disposable non-secret values. The Android production-signing path was then exercised end to end against the real Expo-generated Gradle project using a one-time non-debug certificate: `assembleRelease`, v2 signature verification, pinned certificate comparison, public endpoint embedding and loopback rejection all passed. This disposable key is not production evidence. The development classifier report is `backend-ai/evaluation/development-benchmark-20260810.json`; its development gate passes and its production gate fails closed. |
| CI | green for published `dev` SHA `7c97b40a83ab651f26c04cb61e70d11dc8ec1d32` | Push run `31382275926` passed all nine required jobs, including the native Android candidate build. The resulting APK was independently downloaded: it is structurally valid and v2-signed, but its signer is `CN=Android Debug`; therefore it proves CI installability only and is not a production release APK. Branch rulesets for `dev` and `master` require all nine checks. |
