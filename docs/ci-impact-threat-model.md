...
| TM-007 | Shadow CLI caller | Controls flags or inherited environment | Spoofs PR/feature context on schedule, master or release, or contaminates Actions command files during canonical recomputation | Unsafe selective counterfactual or downstream output mutation | Deterministic-plan trust boundary | Selective results require exact immutable Actions context; local context is full-CI-only; resolver child gets a minimal sanitized environment (`deterministic.py`, `run_ci_impact_shadow.py`) | Future workflow integration does not exist | Keep context-binding and command-file non-mutation tests required for integration | Context-mismatch/full-CI reason and byte-for-byte command-file test | low | high | high |
| TM-008 | Dataset or registry writer | Collection/storage access | Supplies schema-valid but unauthenticated history, labels or receipt lineage | Poisoned future eligibility | Dataset, receipt, candidate lineage | Unknown-only labels and unavailable approval verifier hard-block eligibility | No immutable repo ID, exact `ci.yml` run/event or authenticated collector identity; pagination is not actual cumulative pages; reads precede validation; receipt is not candidate lineage | Authenticate a receipt over immutable repo/workflow-run/event/collector identity and dataset checksum, bind it to candidate lineage, enforce cumulative streaming budgets and validate before use; require approved reviewers and authenticated dual review | Receipt/lineage mismatch, unexpected workflow identity and budget alerts | medium | high | high |
