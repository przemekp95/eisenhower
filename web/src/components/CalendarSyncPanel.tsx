  ...
  const resolve = async (conflict: CalendarConflictDto, strategy: 'eisenhower' | 'google') => {
    try {
      await resolveCalendarConflict(
        ...
        strategy,
        conflict.revision,
        operationId('web-calendar-resolve')
      );
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        {status?.connection ? (
          <button
            type="button"
            onClick={() => void sync()}
            className="min-h-11 rounded-xl border border-cyan-200/30 px-3 py-2 text-sm font-semibold text-cyan-100 hover:bg-white/10"
          >
            {t('calendar.syncNow')}
          </button>
        ) : null}
      ...
      {conflicts.length ? (
        <div className="mt-3 space-y-2">
          ...
          {conflicts.map((conflict) => (
            <article
              key={conflict._id}
              className="rounded-xl border border-amber-200/20 bg-amber-300/5 p-3"
            >
